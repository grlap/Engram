...
| Runtime binding | `ENGRAM_HOME` selects host-local data, `ENGRAM_ACTOR_ID` and `ENGRAM_SESSION_ID` supply asserted principals, and optional `ENGRAM_ACTOR_CONTEXT` adds bounded attribution. | [Host configuration](features/cli-and-mcp.md#host-configuration) |
...
| Gate evidence | `gate` records one bounded pass or fail observation as ordinary work evidence and adds no separate completion barrier. | [Gates, prerequisites, supersession, and project memories](features/local-work-system.md#gates-prerequisites-supersession-and-project-memories) |
