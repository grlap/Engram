...
- every table the file declares, with rows or without, is an ordinary table
  that holds records in the current format (not a search index, one of its
  shadow tables, or a table repair rebuilds); every column it declares, the
  format marker's included, has a place there, and it declares every column
  that table requires;
- everything the file names as left out is something this build leaves out
  too: a table it rebuilds, one of its search indexes or their shadow tables,
  or a row-less entry the file reports as an undeclared index;
...
for is never dropped in silence. Canonical objects keep their ids, links and
bytes. A store written by a design this build no longer knows is refused the
same way; the build that still reads it is kept beside its backups.

Records the store already holds are read as they were stored, including ones
written by operations this build no longer offers: historical `lease_required`
refusal decisions replay unchanged, and `control_operation_results` receipts for
...
A format change edits the schema in place. When existing stores need
converting, the build that makes the change teaches import exactly that
difference (a mapped column, a record reshaped under its existing id, or data
named as retired), reports each with its counts, and keeps record ids and links
unchanged. That conversion code is removed once every live store has been
converted. Export never performs these changes.
Derived tables count too: dropping, renaming or reclassifying a table the build
rebuilds (turning it into a record table, or a record table into one), or
dropping or renaming a search index or one of its shadow tables, makes the
previous build's export carry or leave out something this build classifies
differently, so import refuses it until the change names it. A derived table a
new build only adds needs nothing: the previous export does not name it, and
repair builds it.
...
4. Inspect the import report, including `left_out`, before deciding to
   activate it. Move the old file aside as the backup **together
...
imported store is the same store on the same host: do not run it beside its
source, and do not treat it as authority on another host.
