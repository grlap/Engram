...
| Raw CLI `core {propose,update,complete,handoff} --input` | 2 MiB for the entire JSON input, including whitespace, any file BOM, and the outer envelope; checked before decoding. One leading UTF-8 BOM is accepted only for `@file`. This is a raw transport ceiling shared by those four operations, not a typed semantic maximum. |
